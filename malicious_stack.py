...
# ---- NEW FEATURES ADDED IN 8TH SEM ----

# Feature 1: Domain Entropy
# Measures randomness in domain name
# Legitimate domains (google, amazon) have low entropy
# Malicious/DGA generated domains (xk92mdf) have high entropy
import math
from collections import Counter

def domain_entropy(url):
    try:
        domain = urlparse(str(url)).netloc  # extract only domain part
        if not domain:
            return 0
        counts = Counter(domain)  # count frequency of each character
        total = len(domain)
...
# Feature 2: Digit Ratio in Domain
# Measures fraction of digits specifically in domain name
# Attackers write paypa1 instead of paypal (typosquatting)
# Legitimate brand names are words not numbers
def digit_ratio_domain(url):
    try:
        domain = urlparse(str(url)).netloc  # extract only domain part
        if not domain:
            return 0
        digits = sum(1 for c in domain if c.isdigit())  # count digits in domain
        return digits / len(domain)  # return ratio not raw count
    except:
        return 0

# Feature 3: Subdomain Count
# Counts how many subdomains a URL has
# Attackers use paypal.login.verify.steal.com to confuse users
# Legitimate sites rarely have more than 1-2 subdomains
def subdomain_count(url):
    try:
        hostname = urlparse(str(url)).netloc  # extract hostname
        if not hostname:
            return 0
        parts = hostname.split('.')  # split by dot
        return max(0, len(parts) - 2)  # subtract domain + TLD to get subdomain count
    except:
        return 0
...
def extract_features_series(url_series):
    ...
    # 8th sem new features — domain semantic features
    df['domain_entropy'] = df['url'].apply(domain_entropy)        # entropy of domain name
    df['digit_ratio_domain'] = df['url'].apply(digit_ratio_domain) # digit ratio in domain only
    df['subdomain_count'] = df['url'].apply(subdomain_count)       # number of subdomains
    return df
...
def predict_url_readable(url, model_path='stacked_model.pkl'):
    ...
    for name, v in sorted_contrib:
        print(f"  {name}: {v:.4f}")
    # ---- SHAP EXPLAINABILITY (8th sem addition) ----
    # Replaces fake hardcoded heuristic reasons with real mathematical explanation
    # SHAP tells us which features actually caused this specific prediction
    
    import shap
    
    # we use RandomForest for SHAP explanation as it is most stable for tree explainer
    explainer = shap.TreeExplainer(trained_bases['RandomForest'])
    
    # calculate SHAP values for this specific URL
    shap_values = explainer.shap_values(Xf)
    
    # get feature names for display
    feat_names = feats_df.columns.tolist()
    
    # shap_values shape is [n_classes][n_samples, n_features]
    # we pick shap values for predicted class only
     # handle both possible shap output formats
    # multiclass returns list of arrays or single 3D array
    if isinstance(shap_values, list):
        ...
        # 3D array format — shape is (samples, features, classes)
    ...
    # print top 5 features that most influenced this prediction
    print("\nTop features that influenced this prediction (SHAP):")
    for feat, val in shap_pairs_sorted[:5]:
        direction = "towards" if val > 0 else "against"
        print(f"  {feat}: {val:.4f} ({direction} {label_name})")
    # ---- END SHAP ----
    print("=============================================\n")
